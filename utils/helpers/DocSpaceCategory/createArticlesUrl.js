export default function createArticlesUrl(data, pagepath, secondWord) {
    const { category_docspace, for_installation_category, for_userguides_category, for_administration_category, for_configuration_category } = data.attributes;
    const { attributes: categoryAttributes } = category_docspace?.data;
    ...
    } else if (secondWord === "configuration") {
        const { level_2, level_3 } = for_configuration_category;
        levels = [level_2].filter(Boolean).map(level => level.toLowerCase().replace(/ /g, "_"));
        if (level_3) {
            fullPath = `${[categoryAttributes.url, ...levels].join("/")}#${level_3.toLowerCase().replace(/ /g, "_")}`;
        }
