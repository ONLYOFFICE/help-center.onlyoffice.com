...
const getLevel3Data = async (locale, category, url) => {
  ...
  const articleResponse = await fetch(`${CMSConfigAPI}/api/${category === "docs" ? "articles" : "article"}-${categorySlugMany}/?locale=${locale}` +
    `&filters[url][$eq]=${url}` +
