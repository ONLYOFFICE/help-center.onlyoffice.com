const GenerateTreeViewData = (data) => {
  const sortByIconOrPositionOrNameTitle = (a, b) => (b.attributes.icon_small?.data?.attributes?.url ? 1 : 0) - (a.attributes.icon_small?.data?.attributes?.url ? 1 : 0) || (a.attributes.position ?? Infinity) - (b.attributes.position ?? Infinity) || (a.attributes.name || a.attributes.title).localeCompare(b.attributes.name || b.attributes.title);
  ...
  return {
    children: data?.data?.sort(sortByIconOrPositionOrNameTitle).map((item) => {
      ...
      return {
        ...
        children: [...(item.attributes[`category_${slug_id}`]?.data ?? []), ...(item.attributes.articles?.data ?? [])]?.sort(sortByIconOrPositionOrNameTitle).map((item2) => ({
          name: item2.attributes.name || item2.attributes.title,
          url: item2.attributes.url,
          children: [...(item2.attributes[`level_2_${slug_id}`]?.data ?? []), ...(item2.attributes[`article_${slug_id}`]?.data ?? [])].sort(sortByIconOrPositionOrNameTitle).map((item3) => ({
            name: item3.attributes.name || item3.attributes.title,
            url: item3.attributes.url,
            children: [
              ...(item3.attributes[`level_3_${slug_id}`]?.data ?? [])?.sort(sortByIconOrPositionOrNameTitle).map((item4) => ({
                name: item4.attributes.name || item4.attributes.title,
                url: item4.attributes.url,
                children: [
                  ...(item4.attributes[`level_4_${slug_id}`]?.data ?? []).sort(sortByIconOrPositionOrNameTitle).map((item5) => ({
